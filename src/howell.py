	...
	def saveByBoard(self, rounds):
		...
		for b in sorted(boards.keys()):	# b is a set of self.decks
			...
			for r in sorted(boards[b], key=lambda x: x[0]):
				...
				roundRow = r[1]*nTbl*self.decks+3
				...
				competitors.remove(r[0])
				competitors = [x - r[0] for x in competitors]	# turn it into relative ref to "this" column
